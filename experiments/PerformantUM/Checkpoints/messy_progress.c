...
// static inline void load_segment(uint32_t a, uint32_t b, uint32_t c, uint32_t *regs, uint32_t *zero, Instruction **pp, uint32_t val, unsigned *i);
...
// static inline void do_switch(uint32_t opcode, uint32_t a, uint32_t b, uint32_t c, uint32_t *regs, uint32_t *zero, Instruction **pp, uint32_t val, unsigned *i)
...
//     //     /* Load Segment (Memory Module) */
//     //     // get_registers(word, &a, &b, &c);
//     //     // load_segment(a, b, c, regs, zero, pp, val, i);
//     //     load_segment(a, b, c, regs, zero, pp, val);
//     //     break;
//     // case 6:
//     //     /* Bitwise NAND */
...
static inline void fill_cache(Instruction *cache, Instruction **program_pointer)
{
    ...
    *program_pointer += NUM_INSTRS;
    // for (Instruction *pp = *program_pointer; pp < *program_pointer + NUM_INSTRS; pp++) {
    //     cache[]
    // }
    // memcpy(cache, *program_pointer, NUM_INSTRS * sizeof(Instruction));

    // Instruction *last = *program_pointer;

    // for (; *program_pointer < last + NUM_INSTRS; pp++) {
    //     cache
    // }
}
