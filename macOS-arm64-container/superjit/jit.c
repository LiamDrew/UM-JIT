...
// TODO: these two instructions need to be updated
size_t seg_load(void *zero, size_t offset, unsigned a, unsigned b, unsigned c)
{
    uint8_t *p = (uint8_t *)zero + offset;
...
// TODO: these two functions need to be updated
uint32_t map_segment(uint32_t size, uint8_t *umem)
{
...
void *load_program(uint32_t b_val, uint8_t *umem)
{
    ...
    // I have no idea what the fuck is going on here, my brain is completely
    // scrambled
...
//     int result = mprotect(new_zero, new_seg_size * CHUNK, PROT_READ | PROT_EXEC);
//     assert(result == 0);
